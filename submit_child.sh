...
if [ ${does_exist} -eq 0 ]; then
    ...
    thecommand="sbatch -p ${partition_submit} --mem=${childmem_submit} -n 1 -t 12:00:00 -o ${logfile} -J ${thisid} DEPENDENCY ./${childscript} ${phase} ${period} ${gcm} ${period_actual} ${var} ${testing} ${deflate_lvl} ${ntest}"
